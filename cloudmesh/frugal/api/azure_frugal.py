...
#dictionary to convert regions from flavors to pricing...Microsoft is inconsistent...
location_conv_dict = {
...
def get_azure_pricing(refresh = False):
    ...
    pricing = requests.get('https://azure.microsoft.com/api/v3/pricing/virtual-machines/calculator/?culture=en-us&discount=mosp&v=20191002-1500-96990').json()
    ...
    for flavor in azureinfo:
        ...
        if key[0] is 'b':
            #basic
            search = 'linux' + key[6:].replace('_','').replace('-','').replace('promo','') + 'basic'
        elif key[0] is 's':
            #standard
            search = 'linux' + key[9:].replace('_','').replace('-','').replace('promo','') + 'standard'
        elif key[0] is 'l':
            #low_priority
            search = 'linux' + key[13:].replace('_','').replace('-','').replace('promo','') + 'standard'
        else:
